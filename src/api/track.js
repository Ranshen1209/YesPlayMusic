 ...
 * @param {string|number} [brOverride] - 覆盖默认的 musicQuality 设置，用于下载等场景
 */
export function getMP3(id, brOverride) {
  const getBr = () => {
    // 当返回的 quality >= 400000时，就会优先返回 hi-res
    const quality =
      brOverride ?? store.state.settings?.musicQuality ?? '320000';
    return String(quality) === 'flac' ? '350000' : quality;
  };
